...
public class Add_Material_with_Storage extends TestBase
{
	@Test(priority = 1)
	public void AddMaterialWithStorage() throws Exception
	{
		...
		System.out.println("Add Material confirmation modal displayed successfully");
	...
	@Test(priority = 2)
	public void AddMaterial_With_Level2_Storage() throws Exception
	{
		SoftAssert softAssertion= new SoftAssert();
		Library TodayDate = new Library();
		...
		if(locationsName.length <=1 )
			Assert.fail("The input does not contain sub-storage names.");
		
		System.out.println(locationsName[0]);
		System.out.println(locationsName[1]);
		//Login in to application
		init();
		InventoryRegularFunctions login = new InventoryRegularFunctions();
		login.UserLogin(userName,password);	
