...
public class StockroomAdminInvitation_Action extends TestBase{
	
	@Test
	public void StockroomAdminInvitation() throws Exception{
		...
		Reporter.log("Navigate to Mconnected Lab and Invite the newly added Stockroom Admin");
		Thread.sleep(3000);
		driver.switchTo().window(tabs.get(0));
		impliciteWait(5);
		getWebElement("InviteAdminsEmailField").sendKeys(Keys.CONTROL + "V");
		Library lib = new Library();
		lib.Select("InviteAdminsRoleDropdown", 0);
		Thread.sleep(2000);
		getWebElement("InviteAdminsInviteButton").click();
		
		Reporter.log("Navigate to 20minutemail and Register newly added Stockroom Admin");
		Thread.sleep(8000);
		driver.switchTo().window(tabs.get(1));
		Thread.sleep(10000);
